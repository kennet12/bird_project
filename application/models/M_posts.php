...
class M_posts extends M_db
{
	...
	{
		$sql = "SELECT I.*, C.alias AS 'category_alias', '0' AS 'child_num' FROM m_posts AS I INNER JOIN m_posts_categories AS C ON (I.category_id = C.id) WHERE 1 = 1";
		if (!is_null($info)) {
	...
	public function relative_items ($info=null, $ids, $active=null, $limit=null, $offset=null, $order_by=null, $sort_by='DESC') {
		$sql   = "SELECT * FROM m_posts  WHERE 1 = 1";
		foreach ($ids as $id) {
			$sql .= " AND id <> '{$id}'";
		}
		if (!is_null($info)) {
	...
	public function count_relative_items($info=null, $ids, $active=null, $order_by=null, $sort_by='DESC') {
		$sql   = "SELECT COUNT(*) FROM m_posts  WHERE 1 = 1";
		foreach ($ids as $id) {
			$sql .= " AND id <> '{$id}'";
		}
		if (!is_null($info)) {
